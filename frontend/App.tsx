...
const App: React.FC = () => {
  ...
  const handleAddCustomProductToCart = (product: Product) => {
    // This function assumes custom products can only be added when logged out for simplicity
    // A more robust solution would involve a new API endpoint.
     setCartItems(prevCart => {
        // For anon cart, ID can be temporary since it's only client-side
        const newItem: CartItem = { id: Date.now(), product, quantity: 1 };
        return [...prevCart, newItem];
  ...
  const handleAuthSuccess = async (user: User) => {
    ...
    const anonCart: Omit<CartItem, 'id'>[] = JSON.parse(localStorage.getItem(ANONYMOUS_CART_KEY) || '[]')
      // Exclude custom generated products from being merged
      .filter((item: CartItem) => item.product.brand !== 'Nexora Custom');
          ...
          <div>
             <div className="flex flex-col lg:flex-row lg:space-x-8">
              <div className="lg:w-2/3">
                 {!currentUser && (
                    <ProductGenerator onAddCustomProductToCart={handleAddCustomProductToCart} />
                  )}
